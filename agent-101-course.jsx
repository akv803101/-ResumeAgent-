...
Fix: Two-pass extraction — markdown headings first, bold-heading fallback second`
  ...
  // ── T9 ─────────────────────────────────────────────────────────────────────
  {
    id: "T9", badge: "🧠", duration: "8 min",
    title: "Memory & Intelligence Over Time",
    sections: [
      {
        heading: "The Stateless Problem",
        content: `Every LLM API call is stateless by default. The model has no memory of what happened last run, last week, or 100 runs ago. For a one-shot tool this is fine. For an agent used repeatedly, it creates four compounding gaps:

**Gap 1 — No history:** Each call starts from zero.
**Gap 2 — No cross-session persistence:** There's no record of past JDs, scores, or outcomes.
**Gap 3 — No feedback loop:** You can tell the output is wrong, but the agent can't act on that signal.
**Gap 4 — No learning from outcomes:** The same judgment calls are made on run 100 as on run 1.

These aren't theoretical problems. They mean the agent can make the same mistake week after week, flag the same gaps without addressing them, and never develop a sense of what works for this specific user.`
      },
      {
        heading: "The Two-Component Fix",
        content: `The solution is a two-component memory layer that works without a database:

**Component 1 — Persistent JSON store (memory.py)**
A rolling window of the last 50 runs saved to \`memory/runs.json\`. Each entry stores: role targeted, ATS before/after scores, critical gaps, and a user rating (+1 / -1). The file is tiny (~4KB at 50 runs) and lives alongside the code.

\`\`\`
memory/runs.json
[
  { "id": "a3f2b1c4", "timestamp": "2026-04-22T...", "role": "Senior Data Analyst at Stripe",
    "ats_before": 42, "ats_after": 84, "critical_gaps": ["Snowflake", "dbt"], "rating": 1 },
  { "id": "9e1d5c88", ..., "rating": -1 }
]
\`\`\`

**Component 2 — Memory Context Injection**
Before every run, \`get_memory_context()\` reads the JSON and builds a structured block:

\`\`\`
## MEMORY CONTEXT — Resume Tailor Agent History
- [2026-04-20] Senior Data Analyst at Stripe  |  ATS 42 → 84  ✅ User approved
  ↳ Critical gaps: Snowflake, dbt
- [2026-04-18] Product Analyst at Airbnb  |  ATS 38 → 71  ❌ User flagged
  ↳ Critical gaps: SQL window functions, stakeholder alignment

### Learned Patterns
- Approved runs: 1  |  Avg ATS gain: +42 pts
- Recurring unresolved gaps: SQL window functions, stakeholder alignment
  → Address these more directly in bullet rewrites and the summary.
\`\`\`

This block is prepended to the system prompt. The orchestrator's Step 0 reads it and carries insights silently into Steps 1–5.`
      },
      {
        heading: "How the Agent Uses Memory",
        content: `The memory skill (Step 0) is advisory — it never overrides the actual JD or resume. It guides:

**Style preference:** If approved runs had impact-first summaries, prefer that style unless the current JD clearly calls for something different.

**Gap emphasis:** If "Snowflake" appears in 3 flagged runs' critical gaps, address it more directly this run — bridge it via transferable skills, surface it higher in the gap analysis.

**ATS target:** If approved runs averaged +42 pts gain, that's the soft benchmark for this run.

**What memory never does:**
- Fabricate skills or experience not in the resume
- Override what's actually in the current JD
- Produce any visible output (Step 0 is silent)
- Invent patterns not present in the actual run data

This makes the memory layer safe by design. It shifts emphasis and style, not content.`
      },
      {
        heading: "The Feedback Loop",
        content: `After every run, a 👍 / 👎 widget appears below the download buttons. The user rates the quality of the output. The rating is stored against that run's ID in \`runs.json\`.

The next run reads those ratings and adjusts:

\`\`\`
Run 1: Senior Data Analyst → 42 → 84 ✅
Run 2: Product Analyst    → 38 → 71 ❌ (SQL gap not addressed well)
Run 3: Data Scientist     → agent sees "recurring gap: SQL" → bridges it more directly
\`\`\`

This is not fine-tuning. The model weights don't change. What changes is the **context** the model receives before doing any work — which is often enough to meaningfully shift output quality.

The general pattern (persist outcomes → inject as context → rate new output → repeat) applies to any agent you build where quality improves with iteration:

\`\`\`
Contract analyst   → flag clauses that were rejected → avoid same patterns next run
Cold email agent   → track reply rates by hook type → favour highest-performing style
Interview coach    → note questions user struggled with → drill those harder next session
\`\`\``
      }
    ]
  }
...
- Memory layer: save past runs + feedback → agent improves over time ✅ (see Build L11)
...
Same pattern: skills + orchestrator + thin wrapper + deploy.`
  ...
  // ── BUILD LESSON 11 ────────────────────────────────────────────────────────
  {
    id: 11, phase: 3, badge: "🧠", duration: "10 min",
    ...
    checklistItems: [
      ...
      "app.py wired: memory injected into prompt, runs saved, feedback buttons showing",
    ],
    sections: [
      {
        heading: "The Four Intelligence Gaps",
        content: `Every API call is stateless. The agent has no memory of yesterday, last week, or 100 runs ago. This creates four gaps:

**Gap 1:** No conversation history — each call starts at zero.
**Gap 2:** No cross-session memory — no file, no record of past runs or scores.
**Gap 3:** No feedback loop — you can tell the output is wrong, but the agent can't act on that.
**Gap 4:** No learning from outcomes — same judgment calls on run 100 as run 1.

The fix is a two-file memory layer: \`memory.py\` (persistence) + \`skills/memory_context.md\` (injection).`
      },
      {
...
def get_memory_context(n: int = 6) -> str:
    ...
    # builds and returns a structured MEMORY CONTEXT block
    # (see memory.py for full implementation)
\`\`\``
      },
      {
        heading: "Step 0: The Memory Skill",
        content: `Create **skills/memory_context.md**:

\`\`\`markdown
# Memory Context Skill

## Role
You are the Memory Analyst. Read the MEMORY CONTEXT block
(if present at the top of the system prompt) and carry
personalisation insights silently into Steps 1–5.

## What to Extract
- Approved runs (✅): ATS gain range, role patterns, bridged gaps
- Flagged runs (❌): recurring unresolved gaps → address more directly
- No rated runs: proceed neutrally, no personalisation

## Output
Do NOT produce any visible output.
Memory is advisory — never override JD or resume content.
Never fabricate patterns not present in the data.
\`\`\`

Then add **Step 0** to **agents/resume_tailor_agent.md** (before Step 1):

\`\`\`markdown
### Step 0: Load Memory Context (if available)
Check whether a MEMORY CONTEXT block is present at the top of the system prompt.
If YES → run the memory_context skill. Carry insights into Steps 1–5 silently.
If NO memory context → skip entirely and proceed to Step 1.
Never fabricate or invent memory.
\`\`\``
      },
      {
        heading: "Wiring It Into app.py",
        content: `Three additions to **app.py**:
...
**2 — Inject memory before API call:**
\`\`\`python
base_system = load_prompts()          # @st.cache_data — static
mem_context  = mem.get_memory_context()  # NOT cached — fresh each run
system = (
    mem_context + "\\n\\n---\\n\\n" + base_system
...
**3 — Save run + feedback buttons (after result):**
\`\`\`python
...
run_id, cur_rating = st.session_state.get("run_id"), st.session_state.get("run_rating")
if cur_rating is None:
    ...
    st.info("📝 Noted — future runs will adjust.")
\`\`\`

**Key design choice:** \`load_prompts()\` is cached (static files). \`get_memory_context()\` is NOT cached — it must see feedback from earlier runs in the same session.`
      }
    ]
  }
];
...
const quickRefCommands = [
  ...
  { label: "Inspect memory runs",     cmd: "cat memory/runs.json  # view all saved runs and ratings" },
];
